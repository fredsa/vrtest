	...
	public static SteamVR instance {
		get {
			...
			if (!Application.isPlaying) {
				Debug.Log ("SteamVR: !Application.isPlaying");
				return null;
			}
#endif
			if (!enabled) {
				Debug.Log ("SteamVR: !enabled");
				return null;
			}

			if (_instance == null) {
				Debug.Log ("SteamVR: _instance = CreateInstance()");
				_instance = CreateInstance ();

				// If init failed, then auto-disable so scripts don't continue trying to re-initialize things.
				if (_instance == null) {
					Debug.LogError ("SteamVR: _enabled = false (Init failed!)");
					_enabled = false;
				}
			}
